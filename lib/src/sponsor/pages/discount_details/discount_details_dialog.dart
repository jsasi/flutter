...
class DialogSavePic extends StatelessWidget {
 ...
 GlobalKey  _repaintKey = GlobalKey();
  ...
  void _saveImage (BuildContext context) async {
    PermissionStatus permission = await BwPermissionBiz.requestSavePhotosPermission(context);
    if (permission == PermissionStatus.granted) {
      RenderRepaintBoundary boundary = _repaintKey.currentContext.findRenderObject();
      ui.Image image = await boundary.toImage();
      ByteData byteData = await image.toByteData(format: ui.ImageByteFormat.png);
      Uint8List pngBytes = byteData.buffer.asUint8List();
      bool result = await ImageUtil.saveToGallery(pngBytes);
      if(result) {
        //提示保存成功
        showToast(Strings.savePicSuccessful);
      } else {
        showToast(Strings.savePicFailure);
        //提示保存失败
      }
    }
  }
